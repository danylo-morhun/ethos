...
import { getAccountDetail, getAccountActivity, getSubAccounts } from '@/features/midas/actions/account-detail';
...
export default async function AccountDetailPage({
  ...
  const showBudget = (account.type === 'EXPENSE' || account.type === 'INCOME') && budget != null && budget > 0;
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <div className="mb-2 flex items-center gap-2">
            <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${TYPE_COLOR[account.type]}`}>
              {TYPE_LABEL[account.type]}
            </span>
          </div>
          <h1 className="text-2xl font-bold">{account.name}</h1>
          <p className={`mt-1 text-3xl font-bold tracking-tight ${balance < 0 ? 'text-destructive' : ''}`}>
        ...
        <div className="mb-8 rounded-lg border p-4">
          <div className="mb-2 flex items-center justify-between text-sm">
            ...
            <span className={overBudget ? 'text-destructive' : 'text-muted-foreground'}>
              {formatCurrency(Math.abs(balance), workspace.baseCurrency)} / {formatCurrency(budget!, workspace.baseCurrency)}
            ...
            <div className="rounded-lg border divide-y">
              {subAccounts.map((sub) => (
                <div key={sub.id} className="flex items-center justify-between px-4 py-3">
                  <Link href={`/midas/accounts/${sub.id}`} className="text-sm font-medium hover:underline">
                    {sub.name}
                  </Link>
