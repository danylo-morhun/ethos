...
import { db, tags, transactionTags, workspaces, eq, and } from '@ethos/db';
...
export async function getTags(workspaceId: string): Promise<Tag[]> {
  ...
  return db.select({ id: tags.id, name: tags.name, color: tags.color }).from(tags).where(eq(tags.workspaceId, workspaceId));
}
