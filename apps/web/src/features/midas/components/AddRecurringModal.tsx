  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit, () => toast.error('Check form for errors'))} className="space-y-4">
          <div className="space-y-2">
            <Label>From Account</Label>
            <AccountSelect control={control as never} name="fromAccountId" accounts={[...wallets, ...categories]} placeholder="Select account" error={errs.fromAccountId?.message} />
          </div>
          <div className="space-y-2">
            <Label>To Account</Label>
            <AccountSelect control={control as never} name="toAccountId" accounts={[...wallets, ...categories]} placeholder="Select account" error={errs.toAccountId?.message} />
          </div>
          <div className="space-y-2">
            <Label>Amount</Label>
            <div className="flex gap-2">
              <Input type="number" step="0.01" placeholder="0.00" className="flex-1" {...register('amount', { valueAsNumber: true })} />
              <Controller
                control={control}
                name="currency"
                render={({ field }) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              {errs.startDate && <p className="text-destructive text-[0.8rem]">{errs.startDate.message}</p>}
            </div>
          </div>
          <div className="space-y-2">
