...
import { toggleRecurring, deleteRecurringTransaction } from '@/features/midas/actions/recurring';
...
export function RecurringTransactionsList({
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      ...
      if ('error' in result) toast.error(result.error);
      else { toast.success('Recurring transaction deleted.'); router.refresh(); }
  ...
  return (
    <>
      <div className="divide-y rounded-md border">
        {items.map((rt) => (
          <div key={rt.id} className={`flex items-start justify-between gap-3 px-4 py-3 ${!rt.isActive ? 'opacity-50' : ''}`}>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 text-sm font-medium">
                <span className="truncate">{rt.fromAccountName} → {rt.toAccountName}</span>
                <span className={`shrink-0 rounded-full px-1.5 py-px text-[10px] font-medium border ${rt.isActive ? 'border-green-500/40 text-green-400' : 'border-muted-foreground/30 text-muted-foreground'}`}>
                  {rt.isActive ? 'active' : 'paused'}
                </span>
              </div>
              <p className="mt-0.5 text-xs text-muted-foreground">
                {formatCurrency(Number(rt.amount), rt.currency !== currency ? rt.currency : currency)}
            ...
            <div className="flex shrink-0 items-center gap-1">
              ...
              >
                <HugeiconsIcon icon={rt.isActive ? PauseIcon : PlayCircleIcon} className="h-3.5 w-3.5" />
              </Button>
              <Button
                variant="ghost"
                size="icon"
